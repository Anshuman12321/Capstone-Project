...
class DatabaseStore:
    ...
    def get_user(self, user_id: UUID) -> User | None:
        rs = self._execute("SELECT user_id, username FROM users WHERE user_id = ?", [str(user_id)])
        if not rs.rows:
            return None
        row = rs.rows[0]
        return User(user_id=_as_uuid(row[0]), username=row[1])

    def get_user_by_username(self, username: str) -> User | None:
        normalized = username.strip().lower()
        rs = self._execute(
            "SELECT user_id, username FROM users WHERE LOWER(TRIM(username)) = ? LIMIT 1",
            [normalized],
        )
        if not rs.rows:
            return None
    ...
    def delete_game(self, game_id: UUID) -> None:
        # Keep it simple: delete child rows first.
